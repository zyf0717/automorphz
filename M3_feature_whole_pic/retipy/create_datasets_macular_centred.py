...
LOGGER.info("Macular-centred full image: %d binary, %d artery, %d vein images", len(binary_files), len(artery_files), len(vein_files))

for index, filename in enumerate(binary_files, start=1):
    LOGGER.info("[Macular Full][binary] %d/%d %s", index, len(binary_files), portable_basename(filename))
    ...
    try:
        ...
        LOGGER.exception("[Macular Full][binary] Failed: %s", filename)
...
for index, filename in enumerate(artery_files, start=1):
    LOGGER.info("[Macular Full][artery] %d/%d %s", index, len(artery_files), portable_basename(filename))
    ...
    try:
        ...
        LOGGER.exception("[Macular Full][artery] Failed: %s", filename)
...
for index, filename in enumerate(vein_files, start=1):
    LOGGER.info("[Macular Full][vein] %d/%d %s", index, len(vein_files), portable_basename(filename))
    ...
    try:
        ...
        LOGGER.exception("[Macular Full][vein] Failed: %s", filename)
...
LOGGER.info("Wrote %s", f'{AUTOMORPH_DATA}/Results/M3/Macular_centred/Macular_Measurement.csv')
