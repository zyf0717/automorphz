...
LOGGER.info("Macular-centred zone C: %d binary, %d artery, %d vein images", len(binary_files), len(artery_files), len(vein_files))

for index, filename in enumerate(binary_files, start=1):
    LOGGER.info("[Macular Zone C][binary] %d/%d %s", index, len(binary_files), portable_basename(filename))
    ...
    try:
        ...
        LOGGER.exception("[Macular Zone C][binary] Failed: %s", filename)
...
for index, filename in enumerate(artery_files, start=1):
    LOGGER.info("[Macular Zone C][artery] %d/%d %s", index, len(artery_files), portable_basename(filename))
    ...
    try:
        ...
        LOGGER.exception("[Macular Zone C][artery] Failed: %s", filename)
...
for index, filename in enumerate(vein_files, start=1):
    LOGGER.info("[Macular Zone C][vein] %d/%d %s", index, len(vein_files), portable_basename(filename))
    ...
    try:
        ...
        LOGGER.exception("[Macular Zone C][vein] Failed: %s", filename)
...
LOGGER.info("Wrote %s", f'{AUTOMORPH_DATA}/Results/M3/Macular_centred/Macular_Zone_C_Measurement.csv')
